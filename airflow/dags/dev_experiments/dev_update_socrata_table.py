...
def ingest_into_table(
    ...
    if socrata_metadata.is_geospatial:
        ...
        gdf.to_postgis(name=table_name, schema="data_raw", con=engine, if_exists=if_exists)
        task_logger.info("Successfully ingested data using gpd.to_postgis()")
    else:
        df = pd.read_csv(local_file_path)
        df["ingestion_check_time"] = time_of_check
        df.to_sql(name=table_name, schema="data_raw", con=engine, if_exists=if_exists)
...
@task
def check_metadata(socrata_metadata: SocrataTableMetadata, conn_id: str) -> SocrataTableMetadata:
    task_logger.info(f"Metadata check: freshness_check_id: {socrata_metadata.freshness_check_id}")
    return socrata_metadata
...
def dev_update_socrata_data_table():
    ...
    metadata_preingest = check_metadata(socrata_metadata=metadata_1, conn_id=POSTGRES_CONN_ID)
    metadata_2 = extract_table_freshness_info(metadata_preingest, POSTGRES_CONN_ID)
    metadata_3 = ingest_table_freshness_check_metadata(metadata_2, POSTGRES_CONN_ID)
    metadata_4 = check_metadata(socrata_metadata=metadata_3, conn_id=POSTGRES_CONN_ID)

    metadata_1 >> metadata_preingest >> metadata_2 >> metadata_3 >> metadata_4
...
# check_table_freshness_dag = dev_update_socrata_data_table()
