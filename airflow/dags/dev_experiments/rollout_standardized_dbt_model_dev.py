...
from cc_utils.file_factory import get_table_sqlalchemy_col_objects
...
@task
def format_dbt_stub_for_standardized_stage(table_name: str, conn_id: str, task_logger: Logger) -> List[str]:
...
# full_table_name = f"data_raw.temp_{SOCRATA_TABLE.table_name}"
...
def dev_rollout_standardized_dbt_model():
    ...
    make_standardize_stage_1 = format_dbt_stub_for_standardized_stage(
        table_name="cook_county_parcel_locations",
        conn_id="dwh_db_conn",
        task_logger=task_logger,
    )
    make_standardize_stage_1
