...
def file_ext_branch_router(socrata_metadata: SocrataTableMetadata) -> str:
    ...
    if dl_format.lower() == "geojson":
        return "ingest_geojson_data.drop_temp_table"
    elif dl_format.lower() == "csv":
        return "ingest_csv_data.drop_temp_table"
    else:
        raise Exception(f"Download format '{dl_format}' not supported yet. CSV or GeoJSON for now")


@task
def drop_temp_table(conn_id: str, **kwargs) -> SocrataTableMetadata:
    ti = kwargs["ti"]
...
@task_group
def ingest_geojson_data() -> SocrataTableMetadata:
    drop_temp_geoj_1 = drop_temp_table()
    create_temp_geoj_1 = create_temp_table_for_geojson_data(socrata_metadata=drop_temp_geoj_1)

    drop_temp_geoj_1 >> create_temp_geoj_1
    return create_temp_geoj_1


@task_group
def ingest_csv_data() -> SocrataTableMetadata:
    drop_temp_csv_1 = drop_temp_table()
    create_temp_csv_1 = create_temp_table_for_csv_data(socrata_metadata=drop_temp_csv_1)

    drop_temp_csv_1 >> create_temp_csv_1
    return create_temp_csv_1


@task_group
def load_data_tg(socrata_metadata: SocrataTableMetadata) -> SocrataTableMetadata:
    # load_start_1 = EmptyOperator(task_id="load_data_start", trigger_rule=TriggerRule.NONE_FAILED_OR_SKIPPED)
    ...
    geojson_route_1 = ingest_geojson_data()
    csv_route_1 = ingest_csv_data()

    file_ext_route_1 >> geojson_route_1
    file_ext_route_1 >> csv_route_1
...
def update_cta_bus_stops_table():
    ...
    file_ext_route_1 = file_ext_branch_router(socrata_metadata=extract_data_1)
    geojson_route_1 = ingest_geojson_data()
    csv_route_1 = ingest_csv_data()

    # load_data_tg_1 = load_data_tg(socrata_metadata=extract_data_1)

    # extract_load_task_group_1 = extract_load_task_group(metadata_4)

    # metadata_5 = update_table_metadata_in_db()

    # fresh_source_data_available_1 >> metadata_4 >> extract_load_task_group_1 >> metadata_5
